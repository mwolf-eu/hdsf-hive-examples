...
// import { FontOutliner } from "../libs/three-font-outliner@2.0.0.js";
	...
	parse: function ( text ) {
		...
		function parseNode( node, style ) {
			...
			switch ( node.nodeName ) {
				...
				case 'rect':
					if (node.parentNode.nodeName == 'clipPath') break;
					style = parseStyle( node, style );
				...
				case 'stop':
				case 'defs':
				// these can be parsed out later
				break;

				case 'text':
					// just enough to get d3 axes working
					// node = document.createElementNS("http://www.w3.org/2000/svg", "circle")
					let ff = node.getAttribute('font-family')
					let fs = node.getAttribute('font-size');

					let pos = {};
					let posAttr = ['x', 'dx', 'y', 'dy'];
					posAttr.forEach((a, i) => {
						pos[a] = node.getAttribute(a) || 0;
						if (/[\d.]+em/.test(pos[a])) pos[a] = parseFloat(pos[a]) * fs; // handle XXem
						else pos[a] = parseFloat(pos[a]);
					});

					node.setAttribute('cx',pos.x + pos.dx)
					node.setAttribute('cy',pos.y + (pos.dy * 1.2))
			...
			if ( path ) {
				...
				if ( style.fill !== undefined && style.fill !== 'none' && style.fill != 'transparent') {
		...
		function parsePathNode( node ) {
			...
			// if (node.id.endsWith('-38') && node.id.includes('rectangle')){
			// 	console.log(d); // path decode bug mw
			// }
		...
		*/
		...
		function parseStyle( node, style ) {
			...
			addStyle( 'fill', 'fill' );
			addStyle( 'opacity', 'fillOpacity', clamp );
			...
			// HARVEST: add more styles
			addStyle( 'text-anchor', 'text-anchor' );
			return style;
		}
