...
exports.layoutText = exports.measureText = void 0;
/**
 * Convert the given camelCased string to snake-case
 */
function camelToSnake(str, delimiter = '-') {
    return str.replace(/[A-Z]/g, (letter) => `${delimiter}${letter.toLowerCase()}`);
}
/**
 * Create markup for the given element, children, and attributes
 */
function createElement(element, children, attrs) {
 ...
 */
function measureText(texts, fontAttrs) {
    const text = texts
        .map((text) => {
...
function wrapLineSingle(runs, maxWidth, fontAttrs) {
    const measuredRuns = measureText(runs.map((r) => r.text), fontAttrs)
        .map((measure, index) => {
        return {
            ...runs[index],
            measure,
    ...
    const longestFitting = measuredRuns[longestFittingIndex] || measuredRuns[measuredRuns.length - 1];
    return {
        line: {
 ...
 */
function layoutText(text, constraints, fontAttrs, lineAttrs) {
